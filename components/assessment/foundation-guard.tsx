...
import { isComplete, HEALTH_SYSTEMS, type AssessedSystemKey } from "@/lib/assessment/registry"
import { persist } from "@/lib/assessment/sync"

export function FoundationGuard({ addon, children }: { addon: AssessedSystemKey; children: React.ReactNode }) {
    ...
    <>
      ...
      {addonDone && (
        <div className="fixed inset-x-0 bottom-0 z-50 px-4 pb-5 pt-3" style={{ background: "linear-gradient(to top, var(--background) 70%, transparent)" }}>
          ...
          >
            View your combined report ({HEALTH_SYSTEMS[addon].label}) <ArrowRight size={18} />
          </Link>
        </div>
      )}
    </>
