    ...
    <section className="px-6 py-20 md:py-28">
      <div className="mx-auto max-w-[1100px]">
        ...
        <div className="grid gap-7 md:grid-cols-3">
          {TESTIMONIALS.map((t, index) => (
            <ScrollReveal key={t.name} delay={index * 100}>
              <figure
                className="relative flex h-full flex-col rounded-3xl border border-border/70 p-10 shadow-md"
                style={{ background: "linear-gradient(165deg, #FFFDF7 0%, #FBF7EE 100%)" }}
              >
                <div
                  aria-hidden
                ...
                <div className="flex gap-1" aria-label="5 out of 5 stars">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} size={20} className="fill-current" style={{ color: "var(--icon-yellow)" }} />
                  ))}
                </div>
                <blockquote className="mt-6 flex-1 font-serif text-xl leading-relaxed text-foreground">
                  “{t.quote}”
                </blockquote>
                <figcaption className="mt-7 text-sm font-semibold text-muted-foreground">
                  {t.name}, {t.location}
                </figcaption>
              </figure>
            </ScrollReveal>
        ...
        <ScrollReveal delay={320}>
          ...
          <p className="mt-14 text-center text-xl font-bold text-foreground sm:text-2xl">
