...
const FOOD_SLUGS_WITH_BENEFIT: { slug: string; benefit: string }[] = [
  { slug: "oats", benefit: "Fibre-rich support for microbial diversity" },
  { slug: "kimchi", benefit: "Fermented food that helps feed the gut ecosystem" },
...
export function BipolarFoods() {
  const foods = FOOD_SLUGS_WITH_BENEFIT
    ...
    .filter(Boolean) as (ReturnType<typeof getFoodBySlug> & { benefit: string })[]
        ...
        <ScrollReveal>
          <div className="mb-10">
            ...
            <h2 className="font-serif text-4xl font-semibold text-foreground sm:text-5xl text-balance">
              Foods that feed a more stable system
            </h2>
            <p className="mt-4 max-w-xl text-base text-muted-foreground leading-relaxed">
              These foods support microbial diversity, gut-brain signalling, and inflammation balance —
              each in a different way. They are a starting point, not a protocol.
            </p>
          </div>
        </ScrollReveal>
