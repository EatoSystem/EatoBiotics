/**
 * Section 6 — the Digital Twin. Copied from
 * components/home/digital-twin-section.tsx and kept essentially intact: the
 * orbit diagram, input/output nodes, the 74 system score, the "next best
 * action" card and the 4-up feature strip all work.
 *
 * Changes from the production version:
 *   - opening line added: "Your Score shows where you are. Your Digital Twin
 *     shows what changes." This echoes the §2 bridge and is the second half of
 *     the page's answer to how the Score and the Twin relate.
 *   - CTA becomes the shared one; production reads "Build My Digital Twin",
 *     another variant of the same action
 *   - microcopy aligned to the hero's "about 3 minutes" (production says 5)
 *
 * ── Mobile is already solved upstream ───────────────────────────────────────
 * The brief flagged the orbit as a likely mobile failure — labels positioned
 * around a central graphic. It isn't: this component already ships a
 * `.dt-diagram-mobile` variant (stacked figure, score, then a two-column node
 * grid), swapped for the fixed 580px `.dt-diagram` by app/globals.css:462-467.
 * Those classes are global, so the copy keeps working without touching
 * globals.css — which the brief forbids anyway.
 *
 * Centerpiece is `/images/couple-hero.png` with `mix-blend-mode: multiply`, so
 ...
 *
 * Server component; brand palette only.
 */
...
const FEATURES = [
  { Icon: Shield, title: "Built on science.", sub: "Rooted in the three biotics.", tint: "rgba(76,182,72,0.12)", color: "#3A8535" },
  { Icon: User, title: "Personal to you.", sub: "Your data. Your system. Your journey.", tint: "rgba(76,182,72,0.12)", color: "#3A8535" },
  { Icon: Leaf, title: "Designed to improve.", sub: "Small steps. Big change. Every day.", tint: "rgba(245,197,24,0.16)", color: "#837615", iconColor: "#F5C518" },
  { Icon: RefreshCw, title: "Always learning.", sub: "Your Food System never stands still.", tint: "rgba(245,166,35,0.14)", color: "#95711C" },
]
...
function InputNode({ n }: { n: NodeDef }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 13, transform: `translateX(${n.tx}px)` }}>
      ...
      <span>
        <span style={{ display: "block", fontSize: 13, fontWeight: 700, letterSpacing: "0.08em", color: "#3A8535" }}>{n.label}</span>
...
function OutputNode({ n }: { n: NodeDef }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 14, flexDirection: "row-reverse", textAlign: "right", transform: `translateX(${n.tx}px)` }}>
      ...
      <span>
        <span style={{ display: "block", fontSize: 13, fontWeight: 700, letterSpacing: "0.08em", color: "#95711C" }}>{n.label}</span>
        <span style={{ display: "block", fontSize: 14, color: "#5A6E50" }}>{n.sub}</span>
      </span>
    </div>
  )
...
export function DigitalTwin() {
  return (
    <section id="newdemo-digital-twin" style={{ background: "#FFFFFF", color: "#1A2E12", padding: "96px 24px", boxSizing: "border-box" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div className="dt-grid">
          ...
          <div>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 9, padding: "7px 16px", border: "1px solid #E5E5E5", borderRadius: 9999, background: "#fff" }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 600, letterSpacing: "0.15em", textTransform: "uppercase", color: "#3A8535" }}>Your Digital Twin</span>
            </div>
            ...
            <h2 style={{ fontFamily: "var(--font-lora), Georgia, serif", fontWeight: 700, fontSize: "clamp(2.4rem,3.6vw,3.5rem)", lineHeight: 1.04, letterSpacing: "-0.02em", margin: "28px 0 0" }}>
              ...
              <span style={{ color: "#47A843" }}>Digital</span><br />
              <span style={{ color: "#C38A1F" }}>Twin.</span>
            </h2>

            <div style={{ width: 64, height: 3, borderRadius: 9999, background: "#4CB648", margin: "28px 0" }} />

            {/* The Score→Twin relationship, echoing the §2 bridge line. */}
            <p style={{ fontSize: 20, lineHeight: 1.5, color: "#1A2E12", fontWeight: 600, maxWidth: 400, margin: 0 }}>
              Your Score shows where you are. Your Digital Twin shows what changes.
            </p>

            <p style={{ fontSize: 18, lineHeight: 1.65, color: "#5A6E50", maxWidth: 380, margin: "16px 0 0" }}>
          ...
          <div className="dt-diagram-mobile">
            ...
            {/* Production hard-codes two columns here, whose combined min-content
                (two 56px icons plus labels like "We explain what it means")
                exceeds a 320px viewport and pushes the whole page wider — the
                same overflow production `/` has at that width. One column below
                400px fixes it; two from 400px up, as before. */}
            <div className="grid grid-cols-1 min-[400px]:grid-cols-2" style={{ gap: 18, marginTop: 24, minWidth: 0 }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", marginTop: -8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 20, background: "#fff", border: "1px solid #EDEDED", borderRadius: 20, boxShadow: "0 14px 38px rgba(26,46,18,0.10)", padding: "18px 22px", maxWidth: 520, width: "100%" }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#3A8535" }}>Your Next Best Action</div>
