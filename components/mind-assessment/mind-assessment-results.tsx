...
const BRAIN_STARTER_PACK: Record<string, string[]> = {
  "Sharp Mind":         ["kimchi", "kefir", "asparagus", "blueberries", "wild-salmon", "almonds"],
  "Clear Foundation":   ["kefir", "blueberries", "wild-salmon", "garlic", "oats", "almonds"],
  "Emerging Clarity":   ["yogurt", "oats", "blueberries", "eggs", "garlic", "almonds"],
  "Foggy System":       ["oats", "yogurt", "blueberries", "garlic", "eggs", "kimchi"],
  "Reactive Mind":      ["oats", "yogurt", "garlic", "banana", "eggs", "lentils"],
  "Early Mind Builder": ["oats", "yogurt", "garlic", "banana", "lentils", "eggs"],
}

const DEFAULT_BRAIN_STARTER: string[] = ["oats", "yogurt", "blueberries", "kefir", "garlic", "wild-salmon"]
...
interface MindAssessmentResultsProps {
...
export function MindAssessmentResults({ result, onRetake, leadEmail }: MindAssessmentResultsProps) {
          ...
          <div className="flex flex-col items-center gap-8 sm:flex-row sm:items-start sm:gap-10 lg:gap-16">
            ...
            <div className="w-full max-w-sm flex-1 rounded-2xl border border-border bg-background/80 p-5 backdrop-blur-sm sm:max-w-none">
              <div className="mb-4 flex items-center gap-2">
                ...
                <span className="text-sm font-bold" style={{ color: profile.color }}>{profile.type}</span>
              </div>
            ...
            <div className="rounded-3xl border border-border bg-background overflow-hidden">
              ...
              <div className="p-6 sm:p-8">
                <div className="flex items-center gap-2">
                  <div className="h-3 w-3 rounded-full" style={{ backgroundColor: profile.color }} />
                  <span className="text-base font-bold" style={{ color: profile.color }}>{profile.type}</span>
                </div>
        ...
        <div className="mx-auto max-w-3xl">
          <ScrollReveal>
            ...
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
              How your food habits score across the five areas that matter most for your gut-brain connection.
            </p>
                  ...
                  return (
                    <ScrollReveal key={insight.pillar} delay={i * 60}>
                      <div className="flex gap-4 rounded-2xl border border-[var(--icon-green)]/15 bg-[var(--icon-green)]/5 p-5">
                        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl" style={{ background: insight.gradient }}>
                        ...
                        <div className="flex-1">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <span className="text-xs font-bold tabular-nums" style={{ color: insight.color }}>{insight.score}/100</span>
                          </div>
                          <p className="mt-1 text-sm leading-relaxed text-muted-foreground">{insight.strength}</p>
                          <p className="mt-2 text-xs leading-relaxed text-muted-foreground/70 italic">Next step: {insight.action}</p>
                        </div>
                      </div>
                    </ScrollReveal>
                  )
                      ...
                      <div className="flex gap-4 rounded-2xl border border-border bg-background p-5">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <span className="text-xs font-bold tabular-nums text-muted-foreground">{insight.score}/100</span>
                          </div>
                          <p className="mt-1 text-sm leading-relaxed text-muted-foreground">{insight.opportunity}</p>
                          <p className="mt-2 text-xs leading-relaxed text-muted-foreground/70 italic">This week: {insight.action}</p>
                        </div>
                      </div>
                    </ScrollReveal>
                  )
        ...
        <div className="mx-auto max-w-3xl">
          <ScrollReveal>
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Three specific things you can do this week to improve your gut-brain connection.
            </p>
          </ScrollReveal>
        ...
        <div className="mx-auto max-w-3xl">
          <ScrollReveal>
            <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">
              Your brain food picks
            </p>
            <h2 className="mt-2 font-serif text-2xl font-semibold text-foreground sm:text-3xl">
              Your Brain Starter Pack
            </h2>
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
              Six foods matched to your mind profile — each one directly nourishes the gut-brain
              pathways your score revealed as priorities.
            </p>
          </ScrollReveal>

          <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3">
            {(BRAIN_STARTER_PACK[profile.type] ?? DEFAULT_BRAIN_STARTER).map((slug, i) => {
          ...
          <ScrollReveal delay={400}>
            <div className="mt-6 text-center">
              <Link href="/gut-brain" className="text-sm font-medium text-muted-foreground transition-colors hover:text-foreground">
                Learn the gut-brain science →
              </Link>
            </div>
          </ScrollReveal>
        </div>
