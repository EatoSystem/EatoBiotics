    ...
    <div>
      ...
      <section className="px-6 py-24 md:py-32">
        <div className="mx-auto max-w-[1200px]">
          ...
          <ScrollReveal delay={200}>
            <div className="mt-16 rounded-3xl bg-[#1A1A1A] p-8 sm:p-12">
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {[
                  { value: "32", label: "County AI Agents", desc: "One per Irish county with unique knowledge and conversations" },
                  { value: "<\u20AC25K", label: "Annual Software Cost", desc: "All 32 agents vs \u20AC75\u2013120K traditional outreach" },
                  { value: "\u20AC38M", label: "County Investment Model", desc: "380\u2013520 farms, 720\u20131,050 jobs per county" },
                  { value: "10", label: "Data Layers", desc: "Metadata, intent, sentiment, geography, economics, barriers, opportunities" },
                  { value: "\u20AC30B+", label: "Market Size", desc: "Global regenerative agriculture, 15\u201320% CAGR" },
                  { value: "32 \u2192 32", label: "Scale Path", desc: "Irish counties \u2192 countries via global licensing" },
                ].map((metric) => (
                  <div key={metric.label} className="rounded-xl border border-white/10 bg-white/5 p-6">
                    ...
                    <p className="mt-1 text-xs leading-relaxed text-white/60">{metric.desc}</p>
                  </div>
                ))}
              </div>
            </div>
