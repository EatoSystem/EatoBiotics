...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Validate the product-architecture context (ignored if absent/invalid so
    ...
    // Only the €49 report is sold now; any legacy tier in the request
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      // client_reference_id which has a 200-char Stripe limit. Flat
      // foundation/add-on keys are duplicated for dashboard/webhook readability
      // (the canonical copy lives in result_summary).
      metadata: {
